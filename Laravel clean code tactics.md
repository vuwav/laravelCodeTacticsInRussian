...
Вы можете создавать объекты с текучем API. 
...
Добовлять методы в классы где они нужны это лучше чем создавать по классу на каждое действие, но из-за этого класс может разростись.

Возможно стоит испозовать Трейт. Это не заначить что трейт *обязательно* должен быть переиспользован, нет ничего полхово в том чтобы использовть трейт одноразво.
**Плохо:**
...
Таже тема что и с одноразовыми трейтами.
...
Это может сделать код чише а контроллер к примеру может меньше занть о базе данных.
...
## 👉 Создавай собственные Блэйд дерективы для бизнес логики
...
ВСЕГДА используйте строгое сравнение (=== и !==). Если нужно меняте тип данных перед сравнением. Это лучше чем не безопасный результат от ==
